...
if __name__ == '__main__':
    ...
    options = Params(path)
    options.train_path = options.path_to_annotations
    options.network = options.base_network
    options.num_epochs = options.epochs
    options.dataset = options.dataset_name
    options.optimizers = options.optimizer
    ...
    if not os.path.isdir(options.output_weight_path):
        os.mkdir(options.output_weight_path)
    if not os.path.isdir(os.path.join(options.output_weight_path, options.network)):
        os.mkdir(os.path.join(options.output_weight_path, options.network))
    C.model_path = os.path.join(options.output_weight_path, options.network, options.dataset + ".h5")
    ...
    if options.optimizers == "SGD":
        optimizer = SGD(lr=options.lr, decay=options.decay, momentum=options.momentum)
        optimizer_classifier = SGD(lr=options.lr, decay=options.decay, momentum=options.momentum)
    else:
