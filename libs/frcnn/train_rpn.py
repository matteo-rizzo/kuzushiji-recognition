...
if __name__ == '__main__':
    ...
    ckpt_path = str(os.path.join("ckpt", "frcnn", "rpn", "rpn." + options.network
                                 + ".weights.{epoch:02d}-{loss:.2f}.hdf5"))
    Callbacks = keras.callbacks.ModelCheckpoint(
        ckpt_path, monitor='loss',
        verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=4)
    callback = [Callbacks]
    if len(val_imgs) == 0:
        # assuming you don't have validation data
        history = model_rpn.fit_generator(data_gen_train,
                                          epochs=options.num_epochs, steps_per_epoch=1000,
                                          callbacks=callback)
