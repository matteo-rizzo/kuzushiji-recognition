...
def convert_data(label: [], class_mapping: pd.DataFrame, image_width: int, image_height: int):
    """
...
def get_annotation_data(image_base_name: str,
    ...
    img_width, img_height = get_image_size(os.path.join(path_to_images, to_file_name(image_base_name)))

    # Create a list of lists to store the annotation data
    annotation_data = [convert_data(label, class_mapping, img_width, img_height) for label in all_labels]

    # Create a dataframe to store the whole annotation
    annotation = pd.DataFrame(annotation_data, columns=['class', 'x_c', 'y_c', 'bb_width', 'bb_height'])
